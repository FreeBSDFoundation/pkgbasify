...
local function execute_conversion(workdir, package_list)
	...
	while not os.execute("pkg --rootdir " .. options.rootdir ..
		" install --fetch-only -y -r " .. options.repo_name .. " " .. packages)
	do
	...
	check_err(os.execute("pkg --rootdir " .. options.rootdir ..
		" install --no-repo-update -y -r " .. options.repo_name .. " " .. packages))
